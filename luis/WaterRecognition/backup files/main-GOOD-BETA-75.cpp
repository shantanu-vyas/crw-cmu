...
int main(int argc, char** argv) 
{
    ...
    IplImage* boatFront = cvLoadImage("20110805_032307.jpg");
   ...
   /* skyX = 0; 
        ...
        // We might need to use other smaller quantities (like 5/6 maybe?)
            for (int i = 0; i < boatFront->height/xDivisor;i++)
            {
                for (int j = 0; j < boatFront->width/yDivisor; j++)
                {
                    ...
                    if (column1 > Y-1)
                        column1 = Y-1;
                    I[row1][column1].h = 0;//0;
                    I[row1][column1].s = 0;//255;
                    I[row1][column1].v = 0;//255;
                }
            }
        }
        else
    ...
    cvFloodFill( backUpImage, seed_point, color, cvScalarAll(1.0), cvScalarAll(2), NULL, 8, NULL );
    //convert to to gray to do more obstacle segmentation
    cvCvtColor(backUpImage, grayImage, CV_BGR2GRAY);
    //convert to binary
    cvThreshold(grayImage, bwImage, 50, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);

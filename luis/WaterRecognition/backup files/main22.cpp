...
int main(int argc, char** argv) 
{
    ...
    CvMat* sampleHue = cvCreateMat(arraySize,arraySize,CV_32FC1);
    CvMat* sampleSat = cvCreateMat(arraySize,arraySize,CV_32FC1);
    CvMat* sampleVal = cvCreateMat(arraySize,arraySize,CV_32FC1);
    ...
    KNN knnWaterH(waterTrainingHue, sampleHue, pixelsNumber);
    KNN knnWaterS(waterTrainingSat, sampleSat, pixelsNumber);
    KNN knnWaterV(waterTrainingVal, sampleVal, pixelsNumber);
    //HSV for ground sample
    KNN knnGroundH(groundTrainingHue, sampleHue, pixelsNumber);
    KNN knnGroundS(groundTrainingSat, sampleSat, pixelsNumber);
    KNN knnGroundV(groundTrainingVal, sampleVal, pixelsNumber);
    //HSV for sky sample   
    KNN knnSkyH(skyTrainingHue, sampleHue, pixelsNumber);
    KNN knnSkyS(skyTrainingSat, sampleSat, pixelsNumber);
    KNN knnSkyV(skyTrainingVal, sampleVal, pixelsNumber);
    
    //distance Matrices
    CvMat* distanceWaterH = cvCreateMat(arraySize,arraySize,CV_32FC1);
    CvMat* distanceWaterS = cvCreateMat(arraySize,arraySize,CV_32FC1);
    CvMat* distanceWaterV = cvCreateMat(arraySize,arraySize,CV_32FC1);
    //HSV for ground sample
    CvMat* distanceGroundH = cvCreateMat(arraySize,arraySize,CV_32FC1);
    CvMat* distanceGroundS = cvCreateMat(arraySize,arraySize,CV_32FC1);
    CvMat* distanceGroundV = cvCreateMat(arraySize,arraySize,CV_32FC1);
    //HSV for sky sample   
    CvMat* distanceSkyH = cvCreateMat(arraySize,arraySize,CV_32FC1);
    CvMat* distanceSkyS = cvCreateMat(arraySize,arraySize,CV_32FC1);
    CvMat* distanceSkyV = cvCreateMat(arraySize,arraySize,CV_32FC1);
    ...
    */
    ...
    while (x < X)
    {
        ...
        if (ix == pixelsNumber-1)
        {
           ...
           KNN knnWaterV(waterTrainingVal, sampleVal, pixelsNumber);
           //find distance
           distanceWaterH = knnWaterH.getEucDistance();
           distanceWaterS = knnWaterS.getEucDistance();
           distanceWaterV = knnWaterV.getEucDistance();
           
           KNN knnGroundH(groundTrainingHue, sampleHue, pixelsNumber);
           KNN knnGroundS(groundTrainingSat, sampleSat, pixelsNumber);
           KNN knnGroundV(groundTrainingVal, sampleVal, pixelsNumber);
           //find distance
           distanceGroundH = knnGroundH.getEucDistance();
           distanceGroundS = knnGroundS.getEucDistance();
           distanceGroundV = knnGroundV.getEucDistance();
            ...
            if (cvmGet(skyTrainingHue,0,0)!=0.0 && cvmGet(skyTrainingSat,0,0)!=0.0 && cvmGet(skyTrainingVal,0,0)!=0.0)
            {
               ...
               KNN knnSkyV(skyTrainingVal, sampleVal, pixelsNumber);
               //find distance
               distanceSkyH = knnSkyH.getEucDistance();
               distanceSkyS = knnSkyS.getEucDistance();
               distanceSkyV = knnSkyV.getEucDistance();
            }
            if ((cvmGet(skyTrainingHue ,0,0)!=0) && (cvmGet(skyTrainingSat,0,0)!=0) && (cvmGet(skyTrainingVal,0,0)!=0))
            {
                for (int i=0; i < pixelsNumber;i++)
                {
                    ...
                    {
                        if (cvmGet(distanceGroundH,i,j) > cvmGet(distanceWaterH,i,j))
                        // mark water samples as green
                            comparatorTwo[0][0] = 1;
                        else
                            comparatorTwo[0][0] = 0;
                        if ((cvmGet(distanceGroundS,i,j) > cvmGet(distanceWaterS,i,j)) && (cvmGet(sampleSat,0,1) <= 0.8*180))
                            //mark water samples as green
                            comparatorTwo[0][1] = 1;
                        else
                            comparatorTwo[0][1] = 0;
                        if ((cvmGet(distanceGroundV,i,j) < cvmGet(distanceWaterV,i,j)) && (cvmGet(sampleVal,0,i) >= 0.9*180))
                        //mark water samples as green
                            comparatorTwo[0][2] = 1;
                        else
                            comparatorTwo[0][2] = 0;
                        // similar sky pixels on the water
                        if ((cvmGet(distanceGroundV,i,j) > cvmGet(distanceSkyH,i,j)) && (cvmGet(distanceWaterH,i,j) > cvmGet(distanceSkyH,i,j))) 
                        //mark water samples as green
                            comparatorTwo[1][0] = 1;
                        else
                            comparatorTwo[1][0] = 0;
                        if ((cvmGet(distanceGroundS,i,j) > cvmGet(distanceSkyS,i,j)) && (cvmGet(distanceWaterS,i,j) < cvmGet(distanceSkyS,i,j)))
                        //mark water samples as green
                            comparatorTwo[1][1] = 1;
                        else
                            comparatorTwo[1][1] = 0;
                        if ((cvmGet(distanceGroundV,i,j) > cvmGet(distanceSkyV,i,j)) && (cvmGet(distanceSkyS,i,j) < cvmGet(distanceSkyS,i,j)))
                        //mark water samples as green
                            comparatorTwo[1][2] = 1;
                        else
                            comparatorTwo[1][2] = 0;
                        ...
                        if ((votesSum > 3) && ((cvmGet(sampleSat,0,i) - cvmGet(sampleVal,0,i)) <= 0.1*180)) 
                        {
                            ...
                            if (skyY == pixelsNumber-1)
                            {
                                ...
                                skyY = skyY + 1; 
                            I[x-pixelsNumber+i-1][y-pixelsNumber+j-1].h = (85/255)*180;
                            I[x-pixelsNumber+i-1][y-pixelsNumber+j-1].s = 1*180;
                            I[x-pixelsNumber+i-1][y-pixelsNumber+j-1].v = (128/255)*180;
                        }
                        votesSum = 0;
                    }    
                }
                ...
                for (int i=0;i<pixelsNumber;i++)
                {
                    for (int j=0;j<pixelsNumber;j++)
                    {
                        ...
                        //mark water samples as green
                            ...
                            comparator[0] = 0;
                        if ((cvmGet(distanceGroundS,i,j) > cvmGet(distanceWaterS,i,j)) && (cvmGet(sampleSat,0,i) <= 0.8*180))
                        //mark water samples as green
                            comparator[1] = 1;
                        else
                            comparator[1] = 0;
                        ...
                        //if resampleVal(n,n) == 0
                            ...
                            if (skyY == pixelsNumber-1)
                            {
                                ...
                                skyY = skyY + 1; 
                            I[x-pixelsNumber+i][y-pixelsNumber+j].h = (85/255)*180;
                            I[x-pixelsNumber+i][y-pixelsNumber+j].s = 1*180;
                            I[x-pixelsNumber+i][y-pixelsNumber+j].v = (128/255)*180;
                        }
                        votesSum = 0;
                    }
                }

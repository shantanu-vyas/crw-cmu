...
int main(int argc, char** argv) 
{
    ...
    CvMat* sampleHue = cvCreateMat(arraySize,1,CV_32FC1);
    CvMat* sampleSat = cvCreateMat(arraySize,1,CV_32FC1);
    CvMat* sampleVal = cvCreateMat(arraySize,1,CV_32FC1);
    ...
    for(int i = 0; i < arraySize; i++)
    {
        for (int j=0; j<arraySize;j++)
        {
            cvmSet(sampleHue,i,0,0);
            cvmSet(sampleSat,i,0,0);
            cvmSet(sampleVal,i,0,0);
            
            cvmSet(resampleHue,i,0,0);
            cvmSet(resampleSat,i,0,0);
            cvmSet(resampleVal,i,0,0);
            ...
            if ((cvmGet(skyTrainingHue,0,0)!=0) && (cvmGet(skyTrainingSat,0,0)!=0) && (cvmGet(skyTrainingVal,0,0)!=0))
            {
                ...
                for (int i=0; i < pixelsNumber;i++)
                {
                    for (int j=0;j<pixelsNumber;j++)
                    {
                        ...
                        if ((votesSum > 3) && ((cvmGet(sampleSat,i,0) - cvmGet(sampleVal,i,0)) <= 0.1)) 
                        {
                            // classify pixel as water
                            cvmSet(resampleHue,skyX,skyY,cvmGet(sampleHue,i,0));
                            cvmSet(resampleSat,skyX,skyY,cvmGet(sampleSat,i,0));
                            cvmSet(resampleVal,skyX,skyY,cvmGet(sampleVal,i,0));
                            if (skyY == pixelsNumber-1)
                            {
                                if (skyX == pixelsNumber-1)
                                    skyX = 1;

    ...
    */
    ...
    while (x < X)
    {
        ...
        if (ix == pixelsNumber-1)
        {
            ...
            responseWaterH = knnWaterHue.find_nearest(sampleHue,pixelsNumber,0,0,nearestWaterH,distanceWaterH);
            responseWaterS = knnWaterSat.find_nearest(sampleSat,pixelsNumber,0,0,nearestWaterS,distanceWaterS);
            responseWaterV = knnWaterVal.find_nearest(sampleVal,pixelsNumber,0,0,nearestWaterV,distanceWaterV);
            responseGroundH = knnGroundHue.find_nearest(sampleHue,pixelsNumber,0,0,nearestGroundH,distanceGroundH);
            responseGroundS = knnGroundSat.find_nearest(sampleSat,pixelsNumber,0,0,nearestGroundS,distanceGroundS);
            responseGroundV = knnGroundVal.find_nearest(sampleVal,pixelsNumber,0,0,nearestGroundV,distanceGroundV);
            cout << cvmGet(distanceGroundH,0,0);
            if ((cvmGet(skyTrainingHue,0,0)!=0) && (cvmGet(skyTrainingSat,0,0)!=0) && (cvmGet(skyTrainingVal,0,0)!=0))
            {
                responseSkyH = knnGroundHue.find_nearest(sampleHue,pixelsNumber,0,0,nearestGroundH,distanceSkyH);
                responseSkyS = knnGroundSat.find_nearest(sampleSat,pixelsNumber,0,0,nearestGroundS,distanceSkyS);
                responseSkyV = knnGroundVal.find_nearest(sampleVal,pixelsNumber,0,0,nearestGroundV,distanceSkyV);
                for (int i=0; i < pixelsNumber;i++)
                {
                    for (int j=0;j<pixelsNumber;j++)
                    {
                        ...
                        // mark water samples as green
                            ...
                            comparatorTwo[0][0] = 0;
                        if ((cvmGet(distanceGroundS,i,j) > cvmGet(distanceWaterS,i,j)) && (cvmGet(sampleSat,0,1) <= 0.8))
                            //mark water samples as green
                            comparatorTwo[0][1] = 1;
                        else
                            comparatorTwo[0][1] = 0;
                        if ((cvmGet(distanceGroundV,i,j) < cvmGet(distanceWaterV,i,j)) && (cvmGet(sampleVal,0,i) >= 0.9))
                        //mark water samples as green
                            comparatorTwo[0][2] = 1;
                        else
                            comparatorTwo[0][2] = 0;
                ...
                for (int i=0;i<pixelsNumber;i++)
                {
                    for (int j=0;j<pixelsNumber;j++)
                    {
                        ...
                        //mark water samples as green
                            ...
                            comparator[0] = 0;
                        if ((cvmGet(distanceGroundS,i,j) > cvmGet(distanceWaterS,i,j)) && (cvmGet(sampleSat,0,i) <= 0.8))
                        //mark water samples as green
                            comparator[1] = 1;
                        else
                            comparator[1] = 0;

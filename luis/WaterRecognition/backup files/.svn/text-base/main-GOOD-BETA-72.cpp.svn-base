...
int main(int argc, char** argv) 
{
    ...
    IplImage* boatFront = cvLoadImage("20110805_063440.jpg");
   ...
   for (int i=0; i<boatFront->height;i++)
    ...
    /*while (x < X-10)
    {
        ...
        for (int i = 0; i < boatFront->height/xDiv; i++)
        {
            for(int j = 0; j < boatFront->width/yDiv; j++)
            {
                ...
                //I[row1][column1].h = 0 + paint;
                //I[row1][column1].s = 10 + paint;
                //I[row1][column1].v = 20 + paint;
            }
        }
    ...
    /***********  pixel = (513.9332077469)pow(distance,0.240675506)  *****************/
            ...
            //Ignore everything less than 0.2 meters apart (anything too close to the boat)
            /*if ((X - (row1 -(boatFront->height/xDiv)/2)) > (513.9332077469*pow(0.2,0.240675506)))
            {
                xObstacleDistance = (1.76e-11)*pow(X - (row1 -(boatFront->height/xDiv)/2), 3.99);
                ...
                yObstacleDistance = (1.76e-11)*pow(Y/2 - (column1 - (boatFront->width/yDiv)/2), 3.99);
                //obstacle distance

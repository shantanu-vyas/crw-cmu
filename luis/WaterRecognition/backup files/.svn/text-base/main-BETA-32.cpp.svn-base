                ...
                for (int i=0;i<pixelsNumber;i++)
                {
                    for (int j=0;j<pixelsNumber;j++)
                    {
                        if (abs(nearestGroundH->data.fl[j]-responseGroundH) > abs(nearestWaterH->data.fl[j]-responseWaterH)) 
                            ...
                            comparator[0] = 0;
                        if ((abs(nearestGroundS->data.fl[j]-responseGroundS) > abs(nearestWaterS->data.fl[j]-responseWaterS)))//&& (cvmGet(sampleSat,0,i) <= 0.8*180))
                        
                        //mark water samples as green
                            comparator[1] = 1;
                        else
                            comparator[1] = 0;
                        if (abs(nearestGroundV->data.fl[j]-responseGroundV) < abs(nearestWaterV->data.fl[j]-responseWaterV))//&& (cvmGet(sampleVal,0,i) >= 0.9*180))

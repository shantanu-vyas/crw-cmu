...
int main(int argc, char** argv) 
{
    ...
    CvMat* resampleHue = cvCreateMat(1,arraySize,CV_32FC1);
    CvMat* resampleSat = cvCreateMat(1,arraySize,CV_32FC1);
    CvMat* resampleVal = cvCreateMat(1,arraySize,CV_32FC1);
    //sky training sample
    ...
    */
    ...
    while (x < X)
    {
        ...
        for (int i = 0; i<pixelsNumber;i++)
        {
            //sampleHue->data.fl[i] = (float)I[x][y+i].h;
            //sampleSat->data.fl[i] = (float)I[x][y+i].s;
            //sampleVal->data.fl[i] = (float)I[x][y+i].v;
        ...
        //{
            ...
            responseGroundV = knnGroundVal.find_nearest(sampleVal,pixelsNumber,0,0,nearestGroundV,0);
                for (int i=0;i<pixelsNumber;i++)
                {
                    ...
                    {
                        if (((minH < cvmGet(sampleHue,0,j)) && (maxH > cvmGet(sampleHue,0,j))) || (nearestGroundH->data.fl[j]-responseGroundH > nearestWaterH->data.fl[j]-responseWaterH))
                            //mark water samples as green
                            comparator[0] = 1;
                        else
                            comparator[0] = 0;
                        if (((minS < cvmGet(sampleSat,0,j)) && (maxS > cvmGet(sampleSat,0,j))) || (nearestGroundS->data.fl[j]-responseGroundS > nearestWaterS->data.fl[j]-responseWaterS))
                        //mark water samples as green
                            comparator[1] = 1;
                        else
                            comparator[1] = 0;
                        if (((minV < cvmGet(sampleVal,0,j)) && (maxV > cvmGet(sampleVal,0,j))) || (nearestGroundV->data.fl[j]-responseGroundV < nearestWaterV->data.fl[j]-responseWaterV) )
                        //mark water samples as green
                            comparator[2] = 1;
                        else
                            comparator[2] = 0;
                        ...
                        //sky detection 
                        if ((votesSum > 1) )//&& ((cvmGet(sampleSat,0,j) - cvmGet(sampleVal,0,j) <= 0.1*180)))
                        {
                        //if the sample is water, make a new sample out of the water classified samples
                            cvmSet(resampleHue,0,i,cvmGet(sampleHue,0,i));
                            cvmSet(resampleSat,0,i,cvmGet(sampleSat,0,i));
                            cvmSet(resampleVal,0,i,cvmGet(sampleVal,0,i));
                            //paint the classified water samples red
    ...
    for (int i=0; i<arraySize; i++)
    {
        for (int j=0; j<arraySize; j++)
        {
            elementAverageHue = elementAverageHue + cvmGet(resampleHue,0,j);
            elementAverageSat = elementAverageSat + cvmGet(resampleSat,0,j);
            elementAverageVal = elementAverageVal + cvmGet(resampleVal,0,j);
        }
    }

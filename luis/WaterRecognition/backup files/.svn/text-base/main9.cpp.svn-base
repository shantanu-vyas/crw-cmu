...
int main(int argc, char** argv) 
{
    ...
    /***********************************************************************/
    int pixelsNumber = 20;  //number of pixels for an n x n matrix and # of neighbors
    const int arraySize = pixelsNumber;
    const int threeArraySize = 3*pixelsNumber;
    ...
    for(int i = 0; i < arraySize; i++)
    {
        ...
        {
            cvmSet(groundTrainingHue,i,j,0);
            cvmSet(groundTrainingSat,i,j,0);
            cvmSet(groundTrainingVal,i,j,0);
    
            cvmSet(waterTrainingHue,i,j,0);
            cvmSet(waterTrainingSat,i,j,0);
            cvmSet(waterTrainingVal,i,j,0);            
        }
    }
    for(int i = 0; i < arraySize; i++)
    {
        ...
        {
            cvmSet(sampleHue,i,j,0);
            cvmSet(sampleSat,i,j,0);
            cvmSet(sampleVal,i,j,0);
            
            cvmSet(resampleHue,i,j,0);
            cvmSet(resampleSat,i,j,0);
            cvmSet(resampleVal,i,j,0);
    
            cvmSet(skyTrainingHue,i,j,0);
            cvmSet(skyTrainingSat,i,j,0);
            cvmSet(skyTrainingVal,i,j,0);
        }
    }    
    ...
    */
    ...
    for (int i=0; i<boatFront->height/2;i++)
    {
        ...
        //value
            if (((I[i][j].v >= 0.9) && (I[i][j].s <= 0.8)) 
                && ((I[i][j].h >=170/255) || (I[i][j].h <= 215/255)))
            {
                ...
                I[i][j].h = 69/255;       //H (color)
                I[i][j].s = 0.3;          //S (color intensity)
                I[i][j].v = 0.6;          //V (brightness)
    ...
    for (int i=0;i<threeArraySize;i++)
    {
        for (int j=0;j<arraySize;j++)
        {
            int row1 = ceil(X/1.2866)+ceil(X/5.237)+i+Mw;
            int column1 = ceil(Y/7.0755)+ceil(Y/21.01622)+j+Nw;
        ...
        //patch is green (this is for me to know where the water patch sample is)
            I[row1][column1].h = 85/255;
            I[row1][column1].s = 255/255;
            I[row1][column1].v = 128/255;
        ...
        //horizon
            int row2 = ceil(X/4.7291)+ceil(X/8.3176)+i+Mg;
            int column2 = ceil(Y/7.78378)+ceil(Y/16.54468)+j+Ng;
    ...
    while (x < X)
    {
        ...
        for (int j1=0; j1<pixelsNumber;j1++)
        {
            ...
            {
                cvmSet(sampleHue,ix,j1,I[x-pixelsNumber+ix][j2].h);
                cvmSet(sampleSat,ix,j1,I[x-pixelsNumber+ix][j2].s);
                cvmSet(sampleVal,ix,j1,I[x-pixelsNumber+ix][j2].v);
            }
        }
                ...
                for (int i=0; i < pixelsNumber;i++)
                {
                    for (int j=0;j<pixelsNumber;j++)
                    {
                        ...
                        // mark water samples as green
                            ...
                            comparator2[0][0] = 0;
                        if ((abs(nearestGroundS->data.fl[j]-responseGroundS) > abs(nearestWaterS->data.fl[j]-responseWaterS)) && (cvmGet(sampleSat,i,j) <= 0.8))
                            //mark water samples as green
                            comparator2[0][1] = 1;
                        else
                            comparator2[0][1] = 0;
                        if ((abs(nearestGroundV->data.fl[j]-responseGroundV) < abs(nearestWaterV->data.fl[j]-responseWaterV)) && (cvmGet(sampleVal,i,j) >= 0.9))
                        //mark water samples as green
                            comparator2[0][2] = 1;
                        else
                            comparator2[0][2] = 0;
                        ...
                        {
                            for (int j=0;j<3;j++)
                                votesSum = votesSum + (int*)cvmGet(comparator,i,j);
                        }
                
                        if ((votesSum > 3) && ((cvmGet(sampleSat,i,j) - cvmGet(sampleVal,i,j)) <= 0.1)) 
                        {
                            // classify pixel as water
                            cvmSet(resampleHue,skyX,skyY,cvmGet(sampleHue,i,j));
                            cvmSet(resampleSat,skyX,skyY,cvmGet(sampleSat,i,j));
                            cvmSet(resampleVal,skyX,skyY,cvmGet(sampleVal,i,j));
                            if (skyY == pixelsNumber-1)
                            {
                                if (skyX == pixelsNumber-1)
                                    skyX = 1;
    ...
    elementAverageHue = elementAverageHue / arraySize;
    elementAverageSat = elementAverageSat / arraySize;
    elementAverageVal = elementAverageVal / arraySize;
    for (int i=0; i<threeArraySize; i++)
    {
        for (int j=0; j<arraySize; j++)
        {
    ...
    //the training samples (sort of inefficient, but might do the job...sometimes)
        ...
        { 
            ...
            // learn classifier
            //CvKNearest knn(trainData, trainClasses, 0, false, itemsNumber);
            knnWaterHue(waterTrainingHue, sampleHue, 0, false, pixelsNumber);
            knnWaterSat(waterTrainingSat, sampleSat, 0, false, pixelsNumber);
            knnWaterVal(waterTrainingVal, sampleVal, 0, false, pixelsNumber);
            //HSV for ground sample
            knnGroundHue(groundTrainingHue, sampleHue, 0, false, pixelsNumber);
            knnGroundSat(groundTrainingSat, sampleSat, 0, false, pixelsNumber);
            knnGroundVal(groundTrainingVal, sampleVal, 0, false, pixelsNumber);
            //waterDistance
            for (int i=0;i<pixelsNumber;i++)
            {
                for (int j=0;j<pixelsNumber;j++)
                ...
                // if sample has not been classified as water...
                    if ((cvmGet(sampleHue,i,j)!=85/255) && (cvmGet(sampleSat,i,j)!=1) && (cvmGet(sampleVal,i,j)!=128/255))
                    {
                        if (abs(nearestGroundH->data.fl[j]-responseGroundH) > abs(nearestWaterH->data.fl[j]-responseWaterH)) 
                        //mark water samples as green
                            comparator[0] = 1;

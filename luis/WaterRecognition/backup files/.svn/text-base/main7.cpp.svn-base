    ...
    */
    ...
    while (x < X)
    {
        ...
        for (int j1=0; j1<pixelsNumber;j1++)
        {
            ...
            {
                sampleHue[ix][j1] = imageHue[x-pixelsNumber+ix][j2];
                sampleSat[ix][j1] = imageSat[x-pixelsNumber+ix][j2];
                sampleVal[ix][j1] = imageVal[x-pixelsNumber+ix][j2];
            }
        }
        ...
        if (ix == pixelsNumber-1)
        {
            ...
            {
                knnSkyHue(skyTrainingHue, sampleHue, 0, false, pixelsNumber);
                knnSkySat(skyTrainingSat, sampleSat, 0, false, pixelsNumber);
                knnSkyVal(skyTrainingVal, sampleVal, 0, false, pixelsNumber);
            }
                    ...
                    for (int j=0;j<pixelsNumber;j++)
                    {
                        ...
                        if ((votesSum > 3) && ((sampleSat[i][j] - sampleVal[i][j]) <= 0.1)) 
                        {
                            ...
                            if (skyY == pixelsNumber-1)
                            {
                                ...
                                skyY = skyY + 1; 
                            imageHue[x-pixelsNumber+i-1][y-pixelsNumber+j-1] = 85/255;
                            imageSat[x-pixelsNumber+i-1][y-pixelsNumber+j-1] = 1;
                            imageVal[x-pixelsNumber+i-1][y-pixelsNumber+j-1] = 128/255;
                        }
                        votesSum = 0;
                    }    
                }
                    ...
                    for (int j=0;j<pixelsNumber;j++)
                    {
                        ...
                        //if resampleVal(n,n) == 0
                            ...
                            if (skyY == pixelsNumber-1)
                            {
                                ...
                                skyY = skyY + 1; 
                            imageHue[x-pixelsNumber+i][y-pixelsNumber+j] = 85/255;
                            imageSat[x-pixelsNumber+i][y-pixelsNumber+j] = 1;
                            imageVal[x-pixelsNumber+i][y-pixelsNumber+j] = 128/255;
                        }
                        votesSum = 0;
                    }
                }
    ...
    for (int i=0; i<arraySize; i++)
    {
        for (int j=0; j<arraySize; j++)
        {
            elementAverageHue = elementAverageHue + resampleHue[i][j];
            elementAverageSat = elementAverageSat + resampleSat[i][j];
            elementAverageVal = elementAverageVal + resampleVal[i][j];
        }
    }
    ...
    for (int i=0; i<threeArraySize; i++)
    {
        for (int j=0; j<arraySize; j++)
        {
            waterTrainingHue[i][j] = elementAverageHue;
            waterTrainingSat[i][j] = elementAverageSat;
            waterTrainingVal[i][j] = elementAverageVal;
        }
    }

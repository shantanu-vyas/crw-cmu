...
int main(int argc, char** argv) 
{
    ...
    */
    ...
    while (x < X-1)
    {
        //get a random sample taken from the picture. Must be determined whether
        //it is water or ground
        for (int i = 0; i < ceil(boatFront->height/20); i++)
        {
        ...
        if (votesSum > ceil((boatFront->height/20)*(boatFront->width/20))*(4/5))
        {   
        ...
        y = y + ceil(boatFront->width/20);
        if (y > Y-1)
        {
            x = x + ceil(boatFront->height/20);
            y = 0;
        }
        votesSum = 0;
    }
    ...
    while (counter < Y-1)
    {
        ...
        if ((i == (boatFront->height)/3) && (counter < Y-1))
        {
            ...
            counter = counter + (boatFront->width)/20;
            if (counter > Y)
                counter == Y;
        }
    }
    ...
    while (counter < X -1)
    {
        ...
        if ((j == Y-1) && (counter < X-1))
        {
            j = 0;
            //divide the width by the number of squares
            counter = counter + (boatFront->height)/20;
            if (counter > X)
                counter == X;
        }
    }*/

...
int main(int argc, char** argv) 
{
    ...
    CvMat* nearestWaterH = cvCreateMat(1, itemsNumber, CV_32FC1);
    CvMat* nearestWaterS = cvCreateMat(1, itemsNumber, CV_32FC1);
    CvMat* nearestWaterV = cvCreateMat(1, itemsNumber, CV_32FC1);
    CvMat* nearestGroundH = cvCreateMat(1, itemsNumber, CV_32FC1);
    CvMat* nearestGroundS = cvCreateMat(1, itemsNumber, CV_32FC1);
    CvMat* nearestGroundV = cvCreateMat(1, itemsNumber, CV_32FC1);
    CvMat* nearestSkyH = cvCreateMat(1, itemsNumber, CV_32FC1);
    CvMat* nearestSkyS = cvCreateMat(1, itemsNumber, CV_32FC1);
    CvMat* nearestSkyV = cvCreateMat(1, itemsNumber, CV_32FC1);
    ...
    CvKNearest knnWaterHue(waterTrainingHue, sampleHue, 0, false, itemsNumber);
    CvKNearest knnWaterSat(waterTrainingSat, sampleSat, 0, false, itemsNumber);
    CvKNearest knnWaterVal(waterTrainingVal, sampleVal, 0, false, itemsNumber);
            //HSV for ground sample
    CvKNearest knnGroundHue(groundTrainingHue, sampleHue, 0, false, itemsNumber);
    CvKNearest knnGroundSat(groundTrainingSat, sampleSat, 0, false, itemsNumber);
    CvKNearest knnGroundVal(groundTrainingVal, sampleVal, 0, false, itemsNumber);
            //HSV for sky sample
    CvKNearest knnSkyHue(skyTrainingHue, sampleHue, 0, false, itemsNumber);
    CvKNearest knnSkySat(skyTrainingSat, sampleSat, 0, false, itemsNumber);
    CvKNearest knnSkyVal(skyTrainingVal, sampleVal, 0, false, itemsNumber);    
    ...
    */
    ...
    while (x < X)
    {
        ...
        if (ix == itemsNumber-1)
        {
            ...
            knnWaterHue(waterTrainingHue, sampleHue, 0, false, itemsNumber);
            knnWaterSat(waterTrainingSat, sampleSat, 0, false, itemsNumber);
            knnWaterVal(waterTrainingVal, sampleVal, 0, false, itemsNumber);
            //HSV for ground sample
            knnGroundHue(groundTrainingHue, sampleHue, 0, false, itemsNumber);
            knnGroundSat(groundTrainingSat, sampleSat, 0, false, itemsNumber);
            knnGroundVal(groundTrainingVal, sampleVal, 0, false, itemsNumber);
            //HSV for sky sample
            if (skyTrainingHue[0][0]~=0 && skyTrainingSat[0][0]~=0 && skyTrainingVal[0][0]~=0)
            {
                knnSkyHue(skyTrainingHue, sampleHue, 0, false, itemsNumber);
                knnSkySat(skyTrainingSat, sampleSat, 0, false, itemsNumber);
                knnSkyVal(skyTrainingVal, sampleVal, 0, false, itemsNumber);
            }
            
            //scan nearest neighbors to each pixel
            if ((skyTrainingHue[0][0]~=0) && (skyTrainingSat[0][0]~=0) && (skyTrainingVal[0][0]~=0))
            {
                responseWaterH = knnWaterHue.find_nearest(&sampleHue,itemsNumber,0,0,nearestWaterH,0);
                responseWaterS = knnWaterSat.find_nearest(&sampleSat,itemsNumber,0,0,nearestWaterS,0);
                responseWaterV = knnWaterVal.find_nearest(&sampleVal,itemsNumber,0,0,nearestWaterV,0);
                responseGroundH = knnGroundHue.find_nearest(&sampleHue,itemsNumber,0,0,nearestGroundH,0);
                responseGroundS = knnGroundSat.find_nearest(&sampleSat,itemsNumber,0,0,nearestGroundS,0);
                responseGroundV = knnGroundVal.find_nearest(&sampleVal,itemsNumber,0,0,nearestGroundV,0);
                responseSkyH = knnGroundHue.find_nearest(&sampleHue,itemsNumber,0,0,nearestGroundH,0);
                responseSkyS = knnGroundSat.find_nearest(&sampleSat,itemsNumber,0,0,nearestGroundS,0);
                responseSkyV = knnGroundVal.find_nearest(&sampleVal,itemsNumber,0,0,nearestGroundV,0);
                for (int i =0; i<itemsNumber;i++)
                {
                    for (int j=0;j<itemsNumber;j++)
                    {
                        ...
                        // mark water samples as green
                            ...
                            comparator2[0][0] = 0;
                        if ((abs(nearestGroundS->data.fl[j]-responseGroundS) > abs(nearestWaterS->data.fl[j]-responseWaterS)) && (sampleSat[i][j] <= 0.8))
                            //mark water samples as green
                            comparator2[0][1] = 1;
                        else
                            comparator2[0][1] = 0;
                        if ((abs(nearestGroundV->data.fl[j]-responseGroundV) < abs(nearestWaterV->data.fl[j]-responseWaterV)) && (sampleVal[i][j] >= 0.9))
                        //mark water samples as green
                            comparator2[0][2] = 1;
                        else
                            comparator2[0][2] = 0;

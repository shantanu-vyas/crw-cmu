...
int main(int argc, char** argv) 
{
    ...
    IplImage* backUpImage = cvLoadImage("bigObstacle4.jpg");
...
//int distancePixels = 0;
    ...
    while (counter < Y-1)
    {
        //control thickness of the lines
        ...
        i--;
        if ((i == (boatFront->height)/1.5) && (counter < Y-1))
        {
            i = X;
            //divide the width by the number of squares
            counter = counter + (boatFront->width)/10;
            if (counter > Y)
                counter == Y;
        }
    }
    //horizontal grid lines
    counter = boatFront->height/1.5;
    int j = 0;
    while (counter < X -1)
    {
        //control thickness of the lines
        ...
        if ((j == Y-1) && (counter < X-1))
        {
            j = 0;
            //divide the width by the number of squares
            counter = counter + (boatFront->height)/10;
            if (counter > X)
                counter == X;
        }
    }

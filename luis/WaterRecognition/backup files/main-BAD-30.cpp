...
int main(int argc, char** argv) 
{
    ...
    //int accuracy;
    //responses for each of the classifications
    ...
    */
    ...
    for (int x = X/2; x < X; x++)
    {
        ...
        //{
            ...
            //{
                sampleHue->data.fl[0] = (float)I[x][y].h;
                sampleSat->data.fl[0] = (float)I[x][y].s;
                sampleVal->data.fl[0] = (float)I[x][y].v;
        ...
        //{
            ...
            {*/
                for (int i=0;i<pixelsNumber;i++)
                {
                    for (int j=0;j<pixelsNumber;j++)
                    {
                        if (abs(sampleHue->data.fl[0] - cvmGet(groundTrainingHue,i,j)) > abs(sampleHue->data.fl[0] - cvmGet(groundTrainingHue,i,j)))                
                            //mark water samples as green
                            votesSum = votesSum + 1;
                        //else
                          //  comparator[0] = 0;
                        if (abs(sampleSat->data.fl[0] - cvmGet(groundTrainingSat,i,j)) > abs(sampleSat->data.fl[0] - cvmGet(groundTrainingSat,i,j)))                
                        //mark water samples as green
                            votesSum = votesSum + 1;
                        //else
                          //  comparator[1] = 0;
                        if (abs(sampleVal->data.fl[0] - cvmGet(groundTrainingVal,i,j)) > abs(sampleVal->data.fl[0] - cvmGet(groundTrainingVal,i,j)))
                        ...
                        //sky detection 
                //there are pixelsNumber x pixelsNumber votes. If "yes" votes (1) are
                //more than half the number of votes, then that pixel belongs to
                //the water
                        if (votesSum < pixelsNumber) //&& ((sampleSat[i][j] - sampleVal[i][j]) <= 0.1*180)
                        {
                            I[x][y].h = 60;
                            I[x][y].s = 180;
                            I[x][y].v = 90;

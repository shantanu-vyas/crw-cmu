...
int main(int argc, char** argv) 
{
    ...
    */
    ...
    cvSort(skyTrainingHue, waterTrainingHue, CV_SORT_ASCENDING);
    cvSort(skyTrainingSat, waterTrainingSat, CV_SORT_ASCENDING);
    cvSort(skyTrainingVal, waterTrainingVal, CV_SORT_ASCENDING);
    ...
    IplImage* imageSample7 = cvLoadImage("roundObstacle.jpg");
    ...
    IplImage* imageSample8 = cvLoadImage("roundObstacle2.jpg");
    ...
    float maxH = cvmGet(waterTrainingHue,0,0);
    float maxS = cvmGet(waterTrainingVal,0,0);
    ,maxV, minH, minS,minV;
    for (int i=0; i < threeArraySize; i++)
    {
        for (int j=0; j < arraySize; j++)
    ...
    while (x < X)
    {
        ...
        //{
            ...
            responseGroundV = knnGroundVal.find_nearest(sampleVal,pixelsNumber,0,0,nearestGroundV,0);
                for (int i=0;i<pixelsNumber;i++)
                {
                    for (int j=0;j<pixelsNumber;j++) //arrows pointing left work better
                    {
                        if ((cvmGet(waterTrainingHue,0,0) < cvmGet(sampleHue,0,j)) && (cvmGet(waterTrainingHue,pixelsNumber-1,pixelsNumber-1) > cvmGet(sampleHue,0,j)))
                            //mark water samples as green
                            comparator[0] = 1;
                        else
                            comparator[0] = 0;
                        if ((cvmGet(waterTrainingSat,0,0) < cvmGet(sampleSat,0,j)) && (cvmGet(waterTrainingSat,pixelsNumber-1,pixelsNumber-1) > cvmGet(sampleSat,0,j)))
                        //mark water samples as green
                            comparator[1] = 1;
                        else
                            comparator[1] = 0;
                        if ((cvmGet(waterTrainingVal,0,0) < cvmGet(sampleVal,0,j)) && (cvmGet(waterTrainingVal,pixelsNumber-1,pixelsNumber-1) > cvmGet(sampleVal,0,j)))
                        //mark water samples as green
                            comparator[2] = 1;
                        else
                            comparator[2] = 0;
                        ...
                        if ((votesSum >= 2) )//&& (((cvmGet(sampleSat,0,j) - cvmGet(sampleVal,0,j)) <= 0.1*180))) //|| (cvmGet(sampleVal,0,j) >=0.25*180) || ((cvmGet(sampleSat,0,j) <= 0.6*180) && (cvmGet(sampleVal,0,j)>=0.85*180)) || (cvmGet(sampleVal,0,j) != 1*180)))
                        {

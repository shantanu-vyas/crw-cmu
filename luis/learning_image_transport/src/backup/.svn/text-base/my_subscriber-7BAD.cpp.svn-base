...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  ROS_INFO("Handling imageCallback?");
  /***********************************************************************/
  ...
  */ 
  ...
  /*for (int i=X; i>3*X/6;i--)
  {
      for(int j= (Y/3)-30; j < (Y/3)+30; j++)
      {
          I[i][j].h = 0;
          I[i][j].s = 255;
          I[i][j].v = 255;
  ...
  /**************************************************************************/
	try
  {
    cvShowImage("BoatFront", boatFront);
...
int main(int argc, char **argv)
{
  ...
  while(ros::ok())
  {
  	cvNamedWindow("BoatFront",0);            //0 to maintains sizes regardless of image size
  	cvResizeWindow("BoatFront",400,250);     // new width/heigh in pixels
  	...
  	//image/compressedROS_INFO("Why are you destroying that window?");
  	//camera/image
  	
  	image_transport::Subscriber sub = it.subscribe("image", 1, imageCallback);
  	
  	//ros::spin();
  	//ROS_INFO("Why are you destroying that window?");
  	  	//cvDestroyWindow("BoatFront");
  	loop_rate.sleep();
  }
  return 0;
}

...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
 ...
 /* for (int i=0; i<boatFront->height/3;i++)
  ...
  IplImage* imageSample11 = cvLoadImage("roundObstacle.jpg");
      ...
      for (int i=0;i<pixelsNumber;i++)
      {
          for (int j=0;j<pixelsNumber;j++)
          {
              ...
              // classify pixel as water 
                  I[x-pixelsNumber+i][y-pixelsNumber+j].h = 200;
                  I[x-pixelsNumber+i][y-pixelsNumber+j].s = 200;
                  I[x-pixelsNumber+i][y-pixelsNumber+j].v = 200;
  //                pixelCount++;
                  //fprintf(stderr,"\n Pixel Painted %d\n");
      ...
      {
          x = x + pixelsNumber; /// Because the image comes in flipped
          y = 0;//pixelsNumber-1;
      }
    }
    ...
    while (x < boatFront->height/3)//(x < boatFront->height/1.158)
    {
        ...
        if (votesSum > ((boatFront->height/xDivisor)*(boatFront->width/yDivisor)*(5.5/9)))
        {   

...
for i = 1:X/2          
    for j = 1:Y/3
        ...
        % value
        if ((cropped(i,j,3) >= 0.7) && (cropped(i,j,2) <= 0.1) && ((cropped(i,j,2)>=(240/255)) && (cropped(i,j,2)<=(285/255)))) 
        %The HSV values vary between 0 and 1
            skytrainingHue(i,j) = I(i,j,1);
            skytrainingSat(i,j) = I(i,j,2);
            skytrainingVal(i,j) = I(i,j,3);
            cropped(i,j,1) = 69/255;     %H (color)
            cropped(i,j,2) = 0;          %S (color intensity)
            cropped(i,j,3) = 0;          %V (brightness)
        end
    end
end
%cropped = I;

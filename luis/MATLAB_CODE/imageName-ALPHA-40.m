...
% M controls the x axis (up and down); N controls the y axis (left and right)
Mw = -50; Nw = 500; Mg = -300; Ng = 0;
Mw2 = -50; Nw2 = -900; Mg2 = -300; Ng2 = -900;
% ground sample
groundTrainingHue = zeros(n);
groundTrainingSat = zeros(n);
groundTrainingVal = zeros(n);
groundTrainingHue2 = zeros(n);
groundTrainingSat2 = zeros(n);
groundTrainingVal2 = zeros(n);
% water sample
waterTrainingHue = zeros(n);
waterTrainingSat = zeros(n);
waterTrainingVal = zeros(n);
waterTrainingHue2 = zeros(n);
waterTrainingSat2 = zeros(n);
waterTrainingVal2 = zeros(n);
...
% second water patch sample
waterTrainingHue2(1:3*n,1:n) = cropped(round(X/1.2866)+round(X/5.237)+(1:3*n) + Mw2,round(Y/7.0755)+round(Y/21.01622)+(1:n) + Nw2,1);
waterTrainingSat2(1:3*n,1:n) = cropped(round(X/1.2866)+round(X/5.237)+(1:3*n) + Mw2,round(Y/7.0755)+round(Y/21.01622)+(1:n) + Nw2,2);
waterTrainingVal2(1:3*n,1:n) = cropped(round(X/1.2866)+round(X/5.237)+(1:3*n) + Mw2,round(Y/7.0755)+round(Y/21.01622)+(1:n) + Nw2,3);
...
cropped(round(X/1.286624)+round(X/5.237)+(1:3*n) + Mw2,round(Y/7.07552)+round(Y/21.01622)+(1:n) + Nw2,1) = 85/255;
cropped(round(X/1.286624)+round(X/5.237)+(1:3*n) + Mw2,round(Y/7.07552)+round(Y/21.01622)+(1:n) + Nw2,2) = 255/255;
cropped(round(X/1.286624)+round(X/5.237)+(1:3*n) + Mw2,round(Y/7.07552)+round(Y/21.01622)+(1:n) + Nw2,3) = 128/255;
...
% second ground patch sample
groundTrainingHue2(1:3*n,1:n) = cropped(round(X/4.7291)+round(X/8.3176)+(1:3*n) + Mg2,round(Y/7.78378)+round(Y/16.54468)+(1:n) + Ng2,1);
groundTrainingSat2(1:3*n,1:n) = cropped(round(X/4.7291)+round(X/8.3176)+(1:3*n) + Mg2,round(Y/7.78378)+round(Y/16.54468)+(1:n) + Ng2,2);
groundTrainingVal2(1:3*n,1:n) = cropped(round(X/4.7291)+round(X/8.3176)+(1:3*n) + Mg2,round(Y/7.78378)+round(Y/16.54468)+(1:n) + Ng2,3);
...
cropped(round(X/4.7291)+round(X/8.3176)+(1:3*n) + Mg2,round(Y/7.78378)+round(Y/16.54468)+(1:n) + Ng2,1) = 0;
cropped(round(X/4.7291)+round(X/8.3176)+(1:3*n) + Mg2,round(Y/7.78378)+round(Y/16.54468)+(1:n) + Ng2,2) = 1;
cropped(round(X/4.7291)+round(X/8.3176)+(1:3*n) + Mg2,round(Y/7.78378)+round(Y/16.54468)+(1:n) + Ng2,3) = 1;
...
while (x <= X) 
    ...
    if ((ix == n))
        ...
        [~, waterDistanceHue2] = kNearestNeighbors(waterTrainingHue2,sampleHue,n);
        [~, waterDistanceSat2] = kNearestNeighbors(waterTrainingSat2,sampleSat,n);
        [~, waterDistanceVal2] = kNearestNeighbors(waterTrainingVal2,sampleVal,n);
        ...
        [~, groundDistanceHue2] = kNearestNeighbors(groundTrainingHue,sampleHue,n);
        [~, groundDistanceSat2] = kNearestNeighbors(groundTrainingSat,sampleSat,n);
        [~, groundDistanceVal2] = kNearestNeighbors(groundTrainingVal,sampleVal,n);
        %waterDistance
        for i = 1:n
            for j = 1:n
                ...
                if ((groundDistanceSat(i,j) > waterDistanceSat(i,j)) && (sampleSat(i,j) <= 0.23))
                    % mark water samples as green
                    comparator(1,2) = 1;
                else
                    comparator(1,2) = 0;
                ...
                if ((groundDistanceVal(i,j) < waterDistanceVal(i,j)))% && (sampleVal(i,j) >= 0.73))
                    % mark water samples as green
                    comparator(1,3) = 1;
                else
                    comparator(1,3) = 0;
                ...
                % Second patch analyzer
                if ((groundDistanceHue2(i,j) > waterDistanceHue2(i,j))) 
                    % mark water samples as green
                    comparator(2,1) = 1;
                else
                    comparator(2,1) = 0;
                end
                if ((groundDistanceSat2(i,j) > waterDistanceSat2(i,j)) && (sampleSat(i,j) <= 0.23))
                    % mark water samples as green
                    comparator(2,2) = 1;
                else
                    comparator(2,2) = 0;
                ...
                if ((groundDistanceVal2(i,j) < waterDistanceVal2(i,j)))% && (sampleVal(i,j) >= 0.73))
                    % mark water samples as green
                    comparator(2,3) = 1;
                else
                    comparator(2,3) = 0;
                ...
                %adding each votes for the pixel
                % 4+ votes means it is water
                if sum(sum(comparator)) > 2

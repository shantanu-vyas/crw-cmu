...
for i = 1:150          
    for j = 1:150
        ...
        sample(i,j,1) = cropped(i+1000,j+1500,1);
...
%[x y] = size(sample)
% add the distance to the neighbors. For example, if the sum of the
% distances from the water patch are bigger than the ground patch, then the 
% pixel belongs to the ground (sort of inefficient)

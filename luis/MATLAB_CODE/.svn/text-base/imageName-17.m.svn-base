...
for i = 300:-1:150
    for j = 1000:-1:850
        % water path sample
        groundPatch(i,j,1) = cropped(i,j,1);
        groundPatch(i,j,2) = cropped(i,j,2);
        groundPatch(i,j,3) = cropped(i,j,3);
        
        %path is red (this is for me to know where the ground patch is)
        cropped(i,j,1) = 1;
        cropped(i,j,2) = 1;
        cropped(i,j,3) = 1;
    end

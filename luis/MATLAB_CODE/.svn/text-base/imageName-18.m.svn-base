...
for i = 950:1:1000
    for j = 1100:-1:950
        ...
        waterTraining(i,j,1) = cropped(i,j,1);
        waterTraining(i,j,2) = cropped(i,j,2);
        waterTraining(i,j,3) = cropped(i,j,3);
        
        %path is green (this is for me to know where the water patch is)
        %cropped(i,j,1) = 59/255;
        %cropped(i,j,2) = 208/255;
        %cropped(i,j,3) = 1;
    end
end
% ground sample
for i = 300:-1:150
    for j = 1000:-1:850
        % water patch sample (150 X 150 matirx)
        groundTraining(i,j,1) = cropped(i,j,1);
        groundTraining(i,j,2) = cropped(i,j,2);
        groundTraining(i,j,3) = cropped(i,j,3);
...
for i = 1:1:150          
    for j = 1:1:150
    sample(i,j,1) = cropped(i,j,1);
    sample(i,j,2) = cropped(i,j,2);
    sample(i,j,3) = cropped(i,j,3);
    end
end
% group that identifies the class of the sample patch
% create 150 groups
group = zeros(1,150);
for i = 1:1:150
    group(i,1) = i;
...
Sample = size(sample)
Water = size(waterTraining)
Ground = size(groundTraining)
[waterNeighbors distance] = kNearestNeighbors(sample,groundTraining,50);
w = waterNeighbors
d = distance
cropped = hsv2rgb(cropped);
figure, imshow(cropped);

...
n = 50;      %number of items for an n x n matrix
...
while (ki < X) 
    ...
    %for i = 1:n          
     %   for j = 1:n
            % random sample taken from the picture. Must be determined whether
            % is is water or ground
            %sample(i,j,1) = cropped(i+ki,j+kj,1);
            %sample(1:n,1:n,1) = cropped(ki+(1:n),kj+(1:n),1);
      %  end
    %end
   ...
   % for i = 1:n
    %    for j = 1:n
     %       if groundDistance(i,j) > waterDistance(i,j)
      ...
      %          cropped(i+ki,j+kj,1) = 85/255;
       %         cropped(i+ki,j+kj,2) = 1;
        %        cropped(i+ki,j+kj,3) = 128/255;
            %else
                % red otherwise
    ...
    %}
    % update the iteration variables in the patch (local update)
    %{
    if ((ki <= x) && (y <= Y))
        if ((kj < y) && (y <= Y)) 
            kj = kj + 1;
    ...
    % update the variables in the image (global update)
    if ((y <= Y) && (kj == y))
        y = y + n;
    elseif ((y > Y))
        y = n;
   ...
   % else
    ...
    %}
    kj = kj +1;
    if ((kj == y) && (ki < x))   %local patch update
        kj = y - n;
        ki = ki + 1;
    
    elseif ((kj == y) && (ki == x) && (y < Y-n))
        y = y + n;
        if (y > Y)
            y = Y;
        end
        kj = y - n;
        ki = x - n;
    %restart to the very far left
    elseif ((kj == y) && (ki == x) && (y >= Y))
        y = n;
        kj = y-n;
        x = x + n;
        if (x > X)

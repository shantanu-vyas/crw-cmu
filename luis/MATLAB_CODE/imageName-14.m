...
%get the mean or color average for that patch
waterPatch = mean(waterPatch);
% compare every pixel against that patch

for i = X:-1:1          
    for j = Y/3:-1:1
        % look on how bright the object is
        if cropped(i,j,1) ~= waterPatch(1,500,1)
            cropped(i,j,1) = 0;     %H (color)
        end
        if cropped(i,j,2) ~= waterPatch(1,500,2)
            cropped(i,j,2) = 1;     %S (color intensity)
        end
        if cropped(i,j,3) ~= waterPatch(1,500,3)
            I(i,j,3) = 1;       %V (brightness)
        end
    end
end
cropped = hsv2rgb(cropped);

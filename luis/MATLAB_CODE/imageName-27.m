...
%for i = 1:n
 %   for j = 1:n
        % water patch sample (n X n matirx)
        groundTraining(1:n,1:n) = cropped(299-n+(1:n),999-n+(1:n),1);
        ...
        cropped(299-n+(1:n),999-n+(1:n),1) = 0;
        cropped(299-n+(1:n),999-n+(1:n),2) = 1;
        cropped(299-n+(1:n),999-n+(1:n),3) = 1;
    %end
%end
...
while ((ki < X) && (kj < Y)) 
    ...
    for i1 = 1:n          
        for j1 = 1:n
            ...
            sample(i1,j1) = cropped(ki+i1,kj+j1,1);
    ...
    %for i2 = 1:n
     %   for j2 = 1:n
    if groundDistance(1:n,1:n) > waterDistance(1:n,1:n)
                % mark water samples as green
                cropped(ki+1+(1:n),kj+1+(1:n),1) = 85/255;
                cropped(ki+1+(1:n),kj+1+(1:n),2) = 1;
                cropped(ki+1+(1:n),kj+1+(1:n),3) = 128/255;
    else
                % red otherwise
                cropped(ki+1+(1:n),kj+1+(1:n),1) = 0;
                cropped(ki+1+(1:n),kj+1+(1:n),2) = 1;
                cropped(ki+1+(1:n),kj+1+(1:n),3) = 1;
    end
      %  end
    %end 

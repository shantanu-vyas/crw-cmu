...
waterTraining(1:n,1:n) = cropped(round(X/1.2866)+250-30+(1:n),round(Y/7.0755)-30+400+(1:n),1);
...
cropped(round(X/1.286624)-30+250+(1:n),round(Y/7.07552)-30+400+(1:n),1) = 85/255;
cropped(round(X/1.286624)-30+250+(1:n),round(Y/7.07552)-30+400+(1:n),2) = 255/255;
cropped(round(X/1.286624)-30+250+(1:n),round(Y/7.07552)-30+400+(1:n),3) = 128/255;
% water patch sample (n X n matirx)
groundTraining(1:n,1:n) = cropped(round(X/4.7291)+200-30+(1:n),round(Y/7.78378)-30+500+(1:n),1);
...
cropped(round(X/4.7291)-30+200+(1:n),round(Y/7.78378)-30+500+(1:n),1) = 0;
cropped(round(X/4.7291)-30+200+(1:n),round(Y/7.78378)-30+500+(1:n),2) = 1;
cropped(round(X/4.7291)-30+200+(1:n),round(Y/7.78378)-30+500+(1:n),3) = 1;
    ...
    if ((ix == n))
        ...
        for i = 1:n
            for j = 1:n
                ...
                %elseif (((cropped(x-n+i,y-n+j,3) >= 0.7) && (cropped(x-n+i,y-n+j,2) <= 0.24)) && (groundDistance(i,j) > waterDistance(i,j))) 
                 %   cropped(x-n+i,y-n+j,1) = 85/255;
                  %  cropped(x-n+i,y-n+j,2) = 1;
                   % cropped(x-n+i,y-n+j,3) = 128/255;
                %else

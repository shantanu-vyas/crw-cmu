...
for i = X:-1:1          
    for j = Y/3:-1:1
        ...
        %The HSV values vary between 0 and 1
            ...
            cropped(i,j,3) = 0.8;         %V (brightness)
        end
    end
end
cropped = I;
...
waterTrainingHue(1:n,1:n) = cropped(round(X/1.2866)+round(X/5.237)+(1:n),round(Y/7.0755)+round(Y/21.01622)+(1:n) + 300,1);
waterTrainingSat(1:n,1:n) = cropped(round(X/1.2866)+round(X/5.237)+(1:n),round(Y/7.0755)+round(Y/21.01622)+(1:n) + 300,2);
waterTrainingVal(1:n,1:n) = cropped(round(X/1.2866)+round(X/5.237)+(1:n),round(Y/7.0755)+round(Y/21.01622)+(1:n) + 300,3);        
%patch is green (this is for me to know where the water patch sample is)
cropped(round(X/1.286624)+round(X/5.237)+(1:n),round(Y/7.07552)+round(Y/21.01622)+(1:n) + 300,1) = 85/255;
cropped(round(X/1.286624)+round(X/5.237)+(1:n),round(Y/7.07552)+round(Y/21.01622)+(1:n) + 300,2) = 255/255;
cropped(round(X/1.286624)+round(X/5.237)+(1:n),round(Y/7.07552)+round(Y/21.01622)+(1:n) + 300,3) = 128/255;

% ground patch sample (n X n matirx)
groundTrainingHue(1:n,1:n) = cropped(round(X/4.7291)+round(X/8.3176)+(1:n),round(Y/7.78378)+round(Y/16.54468)+(1:n),1);
groundTrainingSat(1:n,1:n) = cropped(round(X/4.7291)+round(X/8.3176)+(1:n),round(Y/7.78378)+round(Y/16.54468)+(1:n),2);
groundTrainingVal(1:n,1:n) = cropped(round(X/4.7291)+round(X/8.3176)+(1:n),round(Y/7.78378)+round(Y/16.54468)+(1:n),3);        
%patch is red (this is for me to know where the ground patch sample is)
cropped(round(X/4.7291)+round(X/8.3176)+(1:n),round(Y/7.78378)+round(Y/16.54468)+(1:n),1) = 0;
cropped(round(X/4.7291)+round(X/8.3176)+(1:n),round(Y/7.78378)+round(Y/16.54468)+(1:n),2) = 1;
cropped(round(X/4.7291)+round(X/8.3176)+(1:n),round(Y/7.78378)+round(Y/16.54468)+(1:n),3) = 1;
...
while (x <= X) 
    ...
    if ((ix == n))
        ...
        for i = 1:n
            for j = 1:n
                if ((groundDistanceHue(i,j) > waterDistanceHue(i,j)) && ((waterTrainingHue(i,j) < (waterTrainingHue(i,j)+waterTrainingHue(i,j)*0.1)) && (waterTrainingHue(i,j) > (waterTrainingHue(i,j)-waterTrainingHue(i,j)*0.1)))) 

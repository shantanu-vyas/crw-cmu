...
public class CosmFactory {
	 ...
	 * @param p Cosm object which is the gateway to the cosm service
	 ...
	 */
	public static Feed toFeed(Cosm p, String s) {
		Feed f = new Feed(p);
		try {
	...
	CosmException {
		...
		h = n.getNamedItem("created");
		if(h != null) {
			f.setCreated(h.getNodeValue());
		}
	...
	private static void getlocation(Feed f, Document doc)
		...
		if (c != null) {
			...
			;
			if (h != null && !h.getNodeValue().equals("")) {
		...
		for (int i = 0; i < nodeLst.getLength(); i++) {
			...
			if (fstNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				d.setId(fstNode.getAttributes().getNamedItem("id")
						.getNodeValue());
				NodeList lstNmElmntLst = fstElmnt.getElementsByTagName("current_value");
